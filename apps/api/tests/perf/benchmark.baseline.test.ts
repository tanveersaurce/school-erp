...
describe('Phase 24 — Performance Baseline Benchmarking Suite', () => {
  ...
  beforeAll(async () => {
    ...
    // 7. Seed Exam, Schedule & Results
    const exam = await Exam.create({
      tenantId: tenant._id,
      schoolId: school._id,
      academicYearId: academicYear._id,
      title: 'Benchmark Term Examination',
      code: 'EXAM-BENCH',
      term: 'TERM_1',
      examType: 'MID_TERM',
    ...
    // 8. Seed Finance
    const feeCat = await createFeeCategory(tenant._id, school._id);
    await createFeeStructure(tenant._id, school._id, academicYear._id, feeCat._id);
    seededInvoice = await createFeeInvoice(tenant._id, school._id, seededStudents[0]._id, academicYear._id, {
      totalAmount: 200000,
    ...
    const book = await createBook(tenant._id, school._id, { title: 'Benchmark Quantum Physics' });
    await createBookCopy(tenant._id, book._id, { schoolId: school._id, libraryId: lib._id, barcode: 'BC-BENCH-01' });

    // 10. Seed Transport & Hostel
    await createTransportRoute(tenant._id, school._id, { name: 'Benchmark Route Alpha' });
    await createVehicle(tenant._id, school._id);

    const hostel = await createHostel(tenant._id, school._id);
    const room = await HostelRoom.create({ tenantId: tenant._id, hostelId: hostel._id, roomNumber: '101', floor: 1, capacity: 2 });
    const bed = await HostelBed.create({ tenantId: tenant._id, roomId: room._id, bedNumber: '101-A' });
    ...
    // 12. Seed Notifications & Audit Logs (50 records each)
    for (let i = 0; i < 50; i++) {
      await createNotification(tenant._id, admin.user._id, { title: `Bench Notice ${i}` });
      await createAuditLog(tenant._id, {
    ...
    // Capture initial refresh token via login
    const loginRes = await request(app)
      .post('/api/v1/auth/login')
      .send({ email: admin.user.email, password: 'Password123!' });
    const cookies = loginRes.headers['set-cookie'] || [];
    const refreshCookie = Array.isArray(cookies) ? cookies.find((c: string) => c.startsWith('refreshToken=')) : undefined;
    if (refreshCookie) {
  ...
  it('measures all 22 representative operations and outputs baseline report', async () => {
    // 1. Login
    ...
    // Save JSON benchmark for documentation generation
    const jsonPath = path.resolve(process.cwd(), 'tests/perf/baseline_metrics.json');
